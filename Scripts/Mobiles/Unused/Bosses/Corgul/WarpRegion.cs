...
namespace Server.Regions
{
    public class CorgulWarpRegion : Region
    {
        ...
        public void CheckEnter(BaseBoat boat)
        {
            ...
            foreach (PlayerMobile i in boat.GetEntitiesOnPalmierBoard().OfType<PlayerMobile>().Where(pm => pm.NetState != null))
            {
                pms.Add(i);
                PlayerMobile pm = i;
            ...
            if (hasMap)
            {
                ...
                {
                    boat.StopMove(true);
                    boat.SendMessageToAllOnPalmierBoard("The boat has struck a coral reef!");
                }
