...
namespace Server.Items
{
	...
	public abstract class LivreSkills : Item, ICraftable
	{
        ...
        public bool CanRaise(CustomPlayerMobile from, SkillName skill)
        {
			...
			if (m_Level <= 0)
			{
				from.SendMessage("Vous ne pouvez plus augmenter votre comp�tence avec ce livre.");
				return false;
			}
			//else if (from.SkillsTotal >= from.SkillsCap)
			//{
			//	from.SendMessage("Vous ne pouvez plus augmenter votre comp�tence avec ce livre (Total Skill Cap).");
			//	return false;
			//}
            ...
            {
                from.SendMessage("Vous ne pouvez augmenter votre skill. V�rifiez le cadenas.");
                return false;
            }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from is CustomPlayerMobile)
            {
                ...
                {
                    pm.SendMessage("Le livre doit �tre dans votre sac.");
                }
                ...
                {
                    pm.SendMessage("Vous devez �tre dans une biblioth�que pour lire ce livre.");
                }
                else if (!pm.CanBeginAction(typeof(Etude)))
                {
                    pm.SendMessage("Vous ne pouvez �tudier pr�sentement.");
                }
                else if (CanRaise(pm, m_Skill))
                {
                    m_Owner = pm;
                    pm.SendMessage("Vous d�butez votre lecture.");
       ...
       //     from.Validate(ValidateType.Skills);
            ...
            if (lockc == SkillLock.Up)
            {
                ...
                {
                    m_Owner.SendMessage("Vous tournez les pages du livre sans trop comprendre ce qu'il est �crit.");
                }
            }
        }
            ...
            protected override void OnTick()
            {
                if (!m_Owner.CanBeginAction(typeof(Etude)))
                {
                    ...
                    if (m_Livre != null && m_Owner != null)
                    {
                        ...
                        {
                            m_Owner.SendMessage("Le livre doit �tre dans votre sac.");
                        }
                        else if (!m_Livre.IsInLibrary(m_Owner))
                        {
                            m_Owner.SendMessage("Vous devez �tre dans une biblioth�que pour lire ce livre.");
                        }
