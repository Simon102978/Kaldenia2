			...
			try
			{
				foreach ( Item item in World.Items.Values )
				{
					if ( item is AsparagusCrop || item is AsparagusSeedling || item is BananaCrop || item is BananaSeedling || item is BeetCrop || item is BeetSeedling || item is BitterHopsCrop || item is BitterHopsSeedling || item is BlackberryCrop || item is BlackberrySeedling || item is BlackRaspberryCrop || item is BlackRaspberrySeedling ||
						...
						item is CeleryCrop || item is CelerySeedling || item is CoconutCrop || item is CoconutSeedling || item is CornCrop || item is CornSeedling || item is CottonCrop || item is CottonSeedling || item is CranberryCrop || item is CranberrySeedling ||
						...
						item is GreenBeanCrop || item is GreenBeanSeedling || item is GreenSquashCrop || item is GreenSquashSeedling || item is HayCrop || item is HaySeedling || item is HoneydewMelonCrop || item is HoneydewMelonSeedling || item is IrishRoseCrop || item is IrishRoseSeedling ||
						...
						item is OatsCrop || item is OatsSeedling || item is OnionCrop || item is OnionSeedling || item is PansyCrop || item is PansySeedling || item is PeanutCrop || item is PeanutSeedling || item is PeasCrop || item is PeasSeedling || item is PineappleCrop || item is PineappleSeedling ||
						...
						item is RedRaspberryCrop || item is RedRaspberrySeedling || item is RedRoseCrop || item is RedRoseSeedling || item is RiceCrop || item is RiceSeedling || item is SmallBananaCrop || item is SmallBananaSeedling || item is SnapdragonCrop || item is SnapdragonSeedling ||
						...
						item is TeaCrop || item is TeaSeedling || item is TurnipCrop || item is TurnipSeedling || item is WatermelonCrop || item is WatermelonSeedling || item is WheatCrop || item is WheatSeedling || item is WhiteRoseCrop || item is WhiteRoseSeedling || 
