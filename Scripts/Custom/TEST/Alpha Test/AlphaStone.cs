...
{
	public class AlphaStone : Item
	{
		[Constructable]
		public AlphaStone() : base(0xED4) // You can use any item ID you want for the stone
		{
			Movable = false;
		...
		public AlphaStone(Serial serial) : base(serial) { }

		public override void OnDoubleClick(Mobile from)
		{
			if (from is CustomPlayerMobile player)
			{
				from.SendGump(new AlphaStoneGump(player));			
			}
		}
...
{
    public class AlphaStoneGump : BaseProjectMGump
    {
        ...
        {          
            ...
            owner.CloseGump(typeof(AlphaStoneGump));

            int yLine = 0;
      
            AddButtonHtlml(x, y+ yLine * 20,1,"Augmenter le nombre de FE","#FFFFFF");
            yLine++; 

		    AddButtonHtlml(x, y+ yLine * 20,5,"Réduire le nombre de FE","#FFFFFF");
            yLine++; 

            AddButtonHtlml(x, y+ yLine * 20,2,"Reset de Classe","#FFFFFF");
            yLine++; 
        ...
        public override void OnResponse(NetState state, RelayInfo info) //Function for GumpButtonType.Reply Buttons 
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: 
                    {
						m_from.Prompt = new AjustFEPrompt(m_from, 1);
						m_from.SendMessage("Veuillez écrire le nombre que de FE que vous voulez ajouter.");                    
                        break;
                    }
                 case 2: 
                    {
                        m_from.Classe = Classe.GetClasse(0);
                        break;
                    }
                  case 3: 
                    {
                        m_from.Metier = Metier.GetMetier(0);
                        break;
                    }
                  case 4: 
                  {

					 m_from.SendGump(new AlphaSkillsGump(m_from, m_from, null));
                  ...
                  {
        				m_from.Prompt = new AjustFEPrompt(m_from, -1);
						m_from.SendMessage("Veuillez écrire le nombre que de FE que vous voulez réduire.");                    
                        break;
                  }
                  case 6:
                  {
       		...
       		private class AjustFEPrompt : Prompt
		{
			private CustomPlayerMobile m_From;
			private int m_Factor;


			public AjustFEPrompt(CustomPlayerMobile from, int factor)
			{
				m_From = from;
				m_Factor = factor;
			...
			{
				m_From.SendGump(new AlphaStoneGump(m_From));
			}
			...
			public override void OnResponse(Mobile from, string text)
			{
                ...
                if (int.TryParse(text, out Salaire))
                {
					...
					m_From.FERPTotal += Salaire * m_Factor;

					if (m_Factor < 0)
					{
						int NewLvl = 0;
 	
						while (m_From.FERPTotal>= XPLevel.GetLevel(NewLvl).FeRequis && NewLvl <= 30)
						{
							NewLvl++;              
						}
						m_From.Niveau = NewLvl;
					}
 	...
 	public class AlphaEditSkillGump : Gump
    {
        ...
        private readonly AlphaSkillsGumpGroup m_Selected;
        public AlphaEditSkillGump(Mobile from, Mobile target, Skill skill, AlphaSkillsGumpGroup selected)
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            if (info.ButtonID == 1)
            {
                try
                {
                    ...
                    m_From.SendGump(new AlphaSkillsGump(m_From, m_Target, m_Selected));
                }
                catch
                {
                    m_From.SendMessage("Bad format. ###.# expected.");
                    m_From.SendGump(new AlphaEditSkillGump(m_From, m_Target, m_Skill, m_Selected));
                }
            }
            else
            {
    ...
    public class AlphaSkillsGump : Gump
    {
        public static readonly int GumpOffsetX = PropsConfig.GumpOffsetX;
        public static readonly int GumpOffsetY = PropsConfig.GumpOffsetY;
        public static readonly int TextHue = PropsConfig.TextHue;
        ...
        * */
        ...
        private readonly Mobile m_From;
        private readonly Mobile m_Target;
        private readonly AlphaSkillsGumpGroup[] m_Groups;
        private readonly AlphaSkillsGumpGroup m_Selected;
        public AlphaSkillsGump(Mobile from, Mobile target)
            : this(from, target, null)
        {
        }

        public AlphaSkillsGump(Mobile from, Mobile target, AlphaSkillsGumpGroup selected)
            : base(GumpOffsetX, GumpOffsetY)
        {
            m_From = from;
            m_Target = target;

            m_Groups = AlphaSkillsGumpGroup.Groups;
            m_Selected = selected;
            ...
            for (int i = 0; i < m_Groups.Length; ++i)
            {
                ...
                AlphaSkillsGumpGroup group = m_Groups[i];
                ...
                case 0:
                    {
                        ...
                        {
                            AlphaSkillsGumpGroup newSelection = m_Groups[index];

                            if (m_Selected != newSelection)
                                m_From.SendGump(new AlphaSkillsGump(m_From, m_Target, newSelection));
                            else
                                m_From.SendGump(new AlphaSkillsGump(m_From, m_Target, null));
                        }
                ...
                case 1:
                    {
                        if (m_Selected != null && index >= 0 && index < m_Selected.Skills.Length)
                        {
                            ...
                            if (sk != null)
                            {
                                    ...
                                    m_From.SendGump(new AlphaEditSkillGump(m_From, m_Target, sk, m_Selected));
    ...
    public class AlphaSkillsGumpGroup
    {
        private static readonly AlphaSkillsGumpGroup[] m_Groups = new AlphaSkillsGumpGroup[]
        {
            new AlphaSkillsGumpGroup("Crafting", new SkillName[]
            {
                SkillName.Alchemy,
            ...
            new AlphaSkillsGumpGroup("Bardic", new SkillName[]
            {
            ...
            new AlphaSkillsGumpGroup("Magical", new SkillName[]
            {
                SkillName.Chivalry,
                SkillName.EvalInt,
                SkillName.Magery,
            ...
            new AlphaSkillsGumpGroup("Miscellaneous", new SkillName[]
            {
               SkillName.Botanique,
                SkillName.Camping,
                SkillName.Fishing,
            ...
            new AlphaSkillsGumpGroup("Combat Ratings", new SkillName[]
            {
                SkillName.Archery,
                SkillName.Fencing,
                SkillName.Macing,
            ...
            new AlphaSkillsGumpGroup("Actions", new SkillName[]
            {
            ...
            new AlphaSkillsGumpGroup("Lore & Knowledge", new SkillName[]
            {
                SkillName.Anatomy,
        ...
        public AlphaSkillsGumpGroup(string name, SkillName[] skills)
        {
            m_Name = name;
            m_Skills = skills;
        ...
        public static AlphaSkillsGumpGroup[] Groups => m_Groups;
