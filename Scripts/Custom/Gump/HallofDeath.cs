...
{
    public class HallOfDeathGump : BaseProjectMGump
	{
		...
		public HallOfDeathGump(CustomPlayerMobile from,  Dictionary<Mobile, int> dict, int page = 0)
            : base("Hall of Death", 560, 622, true)
        {
			...
			if (Dictionnairy.Count == 0)
			{
				...
				foreach (Account acct in Accounts.GetAccounts())
				{
					for (int i = 0; i < acct.Length; i++)
					{
						...
						{
							mydict.Add(acct[i], acct[i].Deaths);
						}
...
//					AddLabel(x + 10 + colonne * 300, y + 20 + line * 20, 0,  item.Key.Name + "-" + item.Value.ToString());
		...
		public override void OnResponse(NetState sender, RelayInfo info)
        {
			     ...
			     switch (info.ButtonID)
				 {
					 case 1:
						 {
							sender.Mobile.SendGump(new HallOfDeathGump(m_From, Dictionnairy, m_Page - 1));
							 break;
						 }
					 case 2:
						 {
